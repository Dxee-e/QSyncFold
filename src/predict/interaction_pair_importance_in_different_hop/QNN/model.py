...
class Model(nn.Module):
    ...
    def create_qc(self, N: int):
        ...
        def circuit(
            ...
            # return [qml.expval(qml.PauliZ(wires=wires_pos[i])) for i in range(3)], qml.probs(wires=wires_interaction)
            return qml.expval(qml.PauliZ(wires=wires_pos[0])), qml.expval(qml.PauliZ(wires=wires_pos[1])), qml.expval(qml.PauliZ(wires=wires_pos[2])), qml.probs(wires=wires_interaction)
    ...
    def forward(self, protein_sequence: str):
        ...
        interaction_result = torch.zeros(
            (batch, N, N),
            dtype=self.mc.dtype_float,
            device=self.mc.device,
        )
        for i in range(1, N):
            ...
            out0, out1, out2, interaction = out
            out0 = torch.tensor(out0, dtype=self.mc.dtype_float, device=self.mc.device)
            out1 = torch.tensor(out1, dtype=self.mc.dtype_float, device=self.mc.device)
            out2 = torch.tensor(out2, dtype=self.mc.dtype_float, device=self.mc.device)
            interaction = torch.tensor(interaction, dtype=self.mc.dtype_float, device=self.mc.device)
            interaction_result[:, :, i] = interaction[:, :N]
            out = torch.stack([out0, out1, out2], dim=-1).type(self.mc.dtype_float)
        ...
        interaction_result = (
            torch.sort(interaction_result, dim=0)
            .values[int(batch * 0.25) : int(batch * 0.75), :, :]
            .mean(dim=0)
        )
        ...
        return abs_coord, interaction_result

# with torch.no_grad():
#     from config import model_config
#     model_config.init(0)
#     net = Model(model_config)
#     out, interaction_result = net('LAASLACAL')
#     print(out.shape, interaction_result.shape)
