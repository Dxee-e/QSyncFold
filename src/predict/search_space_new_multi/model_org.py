...
def main(seq):
    ...
    import pandas as pd
    from config import model_config
    from fix_coords import fix_coords_by_xyz
    sequence = seq
    output = []
    for i in range(5):
        model_config.__init__()
        model_config.init(i)
        out, _ = Model(model_config)(sequence)
        out = out.cpu().detach().numpy()
        # out = fix_coords_by_xyz(out.cpu().detach().numpy())
        output.append(out)
    ...
    # }
    # df = pd.DataFrame(df)
    # df.to_csv('coords_base.csv', index=False)
    return output
