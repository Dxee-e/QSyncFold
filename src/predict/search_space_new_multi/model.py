...
class Model(nn.Module):
    ...
    def create_qc(self, N: int):
        ...
        def circuit(
            ...
            for i in range(total_num):
                ...
                k_idx = min(abs(i - cur_r_idx), self.mc.qc_k_num - 1)
                for j in range(3):
                    self.record_direct[:, i, j] = interaction_direct[:, i, j] * interaction_k[k_idx, j]
                    qml.ctrl(
                        qml.RY(
            ...
            return [qml.expval(qml.PauliZ(wires=wires_pos[i])) for i in range(3)], qml.state()
    ...
    def forward(self, protein_sequence: str, debug_info: int):
        """forward"""
        # preprocess
        ...
        for i in range(1, N):
            ...
            out, all_states = out
            record_direct = self.record_direct
            # TODO: save
            
            record_all_states[:, i-1, :] = all_states
        ...
        record_all_states = record_all_states.mean(dim=0)
        return abs_coord, record_all_states
